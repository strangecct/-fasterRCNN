...
from my_dataset_m import VOCDataSet_m
...
def create_model(num_classes):
    ...
    backbone.out_channels = 1280  # 设置对应backbone输出特征矩阵的channels _特征矩阵的深度
...
def main():
    ...
    data_transform = {
        "train": transforms.Compose([transforms.ToTensor(),
                                     transforms.RandomHorizontalFlip(0.5)]),#随即水平翻转
        "val": transforms.Compose([transforms.ToTensor()])
    }

    VOC_root_m = "E:/dataset/lineDetect/"  # VOCdevkit
    ...
    train_dataset = VOCDataSet_m(VOC_root_m, "2012", data_transform["train"], "train_m.txt")
    train_sampler = None
    ...
    val_dataset = VOCDataSet_m(VOC_root_m, "2012", data_transform["val"], "val_m.txt")
    ...
    torch.save(model.state_dict(), "./save_weights/pretrain_m.pth")
    ...
    for epoch in range(init_epochs, num_epochs+init_epochs, 1):
        ...
        if epoch in range(num_epochs+init_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, "./save_weights/mobile-model-{}_m.pth".format(epoch))
